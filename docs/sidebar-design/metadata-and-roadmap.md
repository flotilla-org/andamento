...
If no direct tabs are absorbed at that level, the renderer may absorb one child-group path instead: the first child group label is rendered as a header fragment, separated with the rail border character, and an expanded child group can recursively donate its own first direct tab run into the remaining niche. A collapsed absorbed child group only contributes its group fragment, not descendants. Absorbed tab and group fragments keep mouse actions where practical: tabs switch to their tab, and absorbed group fragments toggle their group.
